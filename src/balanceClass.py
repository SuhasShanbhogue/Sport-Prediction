...
def balanceClass():
	...
	for year in tqdm.tqdm(["2011","2012","2013","2014","2015"]):
		df = pd.read_csv(f"../{data_folder}/players_"+year+".csv")
		df1 = pd.read_csv(f"../{data_folder}/team_"+year+".csv")
		...
		#print(df)
		df.to_csv(f"../{data_folder}/playersBal_"+year+".csv",index=False)
		df1.to_csv(f"../{data_folder}/teamBal_"+year+".csv",index=False)
		#print(df['Winner'])
